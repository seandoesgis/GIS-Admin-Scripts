...
parameters = urlp.urlencode({'username' : '', 'password' : '','client' : 'referer','referer': 'https://arcgis.dvrpc.org/dvrpc','expiration': 5,'f' : 'json'})
...
for result in range (1,portaldata['total']+1,100):
    ...
    for item in results:
        ...
        geojsonurl=wfsbaseurl+'?request=GetFeature&service=WFS&typename='+parts[4]+'&outputformat=geojson&srsname=EPSG:4326&format_options=filename:'+parts[4]+'.geojson'
        ...
        #webmapurl='https://arcgis.dvrpc.org/dvrpc/home/webmap/viewer.html?useExisting=0&layers='+id+'&layerId=0'
        portalurl='https://arcgis.dvrpc.org/dvrpc/home/item.html?id='+id
        csvurl=wfsbaseurl+'?request=GetFeature&service=WFS&typename='+parts[4]+'&outputformat=csv%2Bzip&srsname=EPSG:4326&format_options=filename:'+parts[4]+'.zip'
        shpurl=wfsbaseurl+'?request=GetFeature&service=WFS&typename='+parts[4]+'&outputformat=shape%2Bzip&srsname=EPSG:4326&format_options=filename:'+parts[4]+'.zip'
        networkpath= os.path.join('V:\\', parts[3],parts[3]+'.sde',parts[4])
        #spatial=item['extent']
        data={"@type":"dcat:Dataset","identifier":id,"title":title,"description":description,"keyword":keyword,"issued":issued,"modified":modified,"publisher":"DVRPC GIS","contactPoint":{"@type":"vcard:Contact","fn":"Delaware Valley Regional Planning Commission","hasEmail": "mailto:slawrence@dvrpc.org"},"accessLevel":access,"distribution":[{"@type":"dcat:Distribution","title":"Esri Rest API","format":"Esri REST","mediaType":"application/json","accessURL":serviceurl},{"@type":"dcat:Distribution","title":"GeoJSON","format":"GeoJSON","mediaType":"application/geo+json","accessURL":geojsonurl},{"@type":"dcat:Distribution","title":"DVRPC GIS Catalog","mediaType":"text/html","accessURL":portalurl},{"@type":"dcat:Distribution","title":"Metadata XML","mediaType":"application/xml","accessURL":metadataurl},{"@type":"dcat:Distribution","title":"CSV","format":"CSV","mediaType":"text/csv","downloadURL":csvurl},{"@type":"dcat:Distribution","title":"Shapefile","format":"ZIP","mediaType":"application/zip","downloadURL":shpurl},{"@type":"dcat:Distribution","mediaType":"application/vnd.ogc.wfs_xml","format": "OGC WFS","title": "OGC WFS","accessURL":wfscapurl},{"@type":"dcat:Distribution","title":"Network Location","format":"ESRI Feature Class","accessURL":networkpath}]}
        dcat['dataset'].append(data)

with open(r'd:\GIS\share\data.json', 'w') as outfile:
